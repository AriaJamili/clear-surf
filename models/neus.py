...
class TransNeuSModelB(BaseModel):
    ...
    def forward_(self, rays):
        ...
        normal = F.normalize(sdf_grad, p=2, dim=-1)
        alpha = self.get_alpha(sdf, density, normal, t_dirs, dists)[...,None]
        rgb = self.texture(feature_nerf, t_dirs, positions, normal)
...
@models.register('Model-BB')
class TransNeuSModelBB(BaseModel):
    def setup(self):
        self.geometry_nerf = models.make(self.config.geometry_nerf.name, self.config.geometry_nerf)
        self.geometry_neus = models.make(self.config.geometry_neus.name, self.config.geometry_neus)
        self.texture = models.make(self.config.texture.name, self.config.texture)
    ...
    def forward_(self, rays):
        ...
        normal = F.normalize(sdf_grad, p=2, dim=-1)
        alpha = self.get_alpha(sdf, density, normal, t_dirs, dists)[...,None]
        rgb = self.texture(feature_neus, t_dirs, positions, normal)
...
@models.register('Model-E2')
class TransNeuSModelE2(BaseModel):
    def setup(self):
        ...
        self.texture = models.make(self.config.texture.name, self.config.texture)
        self.geometry_enc.contraction_type = ContractionType.AABB
        self.geometry_neus.contraction_type = ContractionType.AABB
        self.geometry_nerf.contraction_type = ContractionType.AABB
        self.geometry_cost.contraction_type = ContractionType.AABB
    ...
    def update_step(self, epoch, global_step):
        ...
        def occ_eval_fn(x):
            enc_features = self.geometry_enc(x)
            sdf, _ = self.geometry_neus(x, enc_features, with_grad=False)
            density, _ = self.geometry_nerf(x, enc_features)
            inv_s = self.variance(torch.zeros([1, 3]))[:, :1].clip(1e-6, 1e6)
            inv_s = inv_s.expand(sdf.shape[0], 1)
            estimated_next_sdf = sdf[...,None] - self.render_step_size * 0.5
            estimated_prev_sdf = sdf[...,None] + self.render_step_size * 0.5
    ...
    def forward_(self, rays):
        ...
        enc_features = self.geometry_enc(positions)
        sdf, feature_sdf, sdf_grad = self.geometry_neus(positions, enc_features, with_grad=True)
        density, feature_density = self.geometry_nerf(positions, enc_features)
    ...
    def regularizations(self, out):
        losses = {}
        losses.update(self.geometry_enc.regularizations(out))
        losses.update(self.geometry_neus.regularizations(out))
        losses.update(self.geometry_nerf.regularizations(out))
        losses.update(self.geometry_cost.regularizations(out))
        losses.update(self.texture.regularizations(out))
        return losses
